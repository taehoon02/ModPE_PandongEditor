...
if(download("/Pandong/ScriptInfo.txt", "https://www.dropbox.com/s/tnazhzuz6hkelyz/ScriptInfo.txt?dl=1")) {
...
toast("이름 바꾸지 마라. 이런 스크립트소스를 제공한 팬동님과 지금까지 팬동에디터를 수정하고 있는 아이린보드를 생각하라고.");
} else {
popup( );
}
}
...
function popup( ) {
var textView=makeTextView("\n   최신 버전이 발견되었습니다. MCPE Korea카페로 이동할까요?\n   (이동)을 누르면 MCPE Korea카페로 이동됩니다.", 15, android.graphics.Color.WHITE, textParams.pop);
...
onClick : function(v){
for(var k=1; players.length; k++) {
Entity.remove(getPlayerByName(players[k].name));
}
toast("모든 플레이어를 추방(킥)하였습니다.");
}
}), null);
