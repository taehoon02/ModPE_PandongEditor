...
* 도움 : Indvel & Dark Tornado & Redstone & Astro
*
...
* 레드스톤님의 Rethod 애드온이 사용되었습니다. 레드스톤님의 Rethod 애드온 적용 후 사용하셔야 모든 기능을 사용하실 수 있습니다.
* 스크립트 제작에 도움을 주신 인디벨님, 다크토네이도님, 레드스톤님 모두 감사드립니다.
*
*
...
maker : "Pandong",
editor : "Irenebode",
helper : "Indvel & Dark Tornado & Redstone & Astro"
};
...
R_Server.sendChat(Player.getName(e)+"님께서 들어오셨습니다", false)
}
}
}
}
...
var file2 = new java.io.File(sdcard+"/Pandong/Irenebode/PandongEditor/140cutout.txt");
file.delete();
file2.delete();
...
if(antiTerrorInfo[6].code == true) {
R_Server.protectBlock(true);
} else if(antiTerrorInfo[6].code == false) { R_Server.protectBlock(false);
}

if(PlayerInvenSave == true) R_Player.enableInventorySave(true);
else if(PlayerInvenSave == false) R_Player.enableInventorySave(false);
...
var serverStart=makeButton("Rethod 서버 설정", new android.view.View.OnClickListener({
onClick : function(v){
serverStartMenu();
}
}), null);
layout.addView(serverStart);
...
R_Server.sendChat("§d[PDE] 어드민이 모든 플레이어를 자신에게로 텔레포트 시켰습니다.", false);
}
}), null);
...
R_Server.sendChat("§d[PDE] 어드민이 모든 플레이어의 체력을 채웠습니다.", false)
}
}), null);
...
R_Server.sendChat("§d[PDE] 어드민이 모든 플레이어를 죽였습니다.", false);
}
}), null);
...
R_Server.sendChat("§d[PDE] 어드민이 모든 플레이어에게 불을 붙였습니다.", false);
}
}), null);
...
R_Server.sendChat("§d[PDE] 어드민이 모든 플레이어에게 번개를 내렸습니다.", false);
}
}), null);
...
R_Server.sendChat("§d[PDE] 어드민이 모든 플레이어에게 약간의 경험치를 지급하였습니다.", false);
}
}), null);
...
R_Server.sendChat("§d[PDE] 어드민이 모든 플레이어를 추방(킥)하였습니다.", false);
}
}), null);
...
R_Server.sendChat("§d[PDE] "+msg, false);
...
try {
var file = new java.io.File(sdcard+"/Pandong/Irenebode/PandongEditor/140cutout.txt");
if(file.exists()) file.delete();

if(download("/Pandong/Irenebode/PandongEditor/140cutout.txt","https://www.dropbox.com/s/ifaswgh9r5gg9ii/140cutout.txt?dl=1")) {
if(readData("140cutout")=="irene") {
Shutdown();
} else {
// null
...
try {
var serverStartMenuWindow = new android.widget.PopupWindow( );

var layout=new android.widget.LinearLayout(ctx);
layout.setOrientation(1);

var title=makeTextView("Rethod 서버 설정", 20, android.graphics.Color.YELLOW, textParams.subtitle);
layout.addView(title);

var S_setName = makeButton("서버 이름 설정", new android.view.View.OnClickListener({
onClick : function(v){
S_setNameDialog();
}
}), null);
layout.addView(S_setName);

var S_setMaxPlayer = makeButton("서버 인원수 설정", new android.view.View.OnClickListener({
onClick : function(v){
S_setMaxPlayerDialog();
}
}), null);
layout.addView(S_setMaxPlayer);

var S_Inven = new android.widget.ToggleButton(ctx);
S_Inven.setTextOn("서버원 인벤세이브 ON");
S_Inven.setTextOff("서버원 인벤세이브 OFF");
S_Inven.setChecked(PlayerInvenSave);
S_Inven.setOnCheckedChangeListener(new android.widget.CompoundButton.OnCheckedChangeListener({
onCheckedChanged : function(toggle, onoff){
if(onoff==true) {
PlayerInvenSave = true;
toast("서버원 인벤세이브가 활성화되었습니다.");
} else if(onoff==false) {
PlayerInvenSave = false;
toast("서버원 인벤세이브가 비활성화되었습니다.");
}
}
}));
layout.addView(S_Inven);

var mainLayout=scrollAndOut(layout, serverStartMenuWindow, false);

setWindow(serverStartMenuWindow, mainLayout, ctx.getWindowManager().getDefaultDisplay().getWidth()*3/7, ctx.getWindowManager().getDefaultDisplay().getHeight(), new android.graphics.drawable.ColorDrawable(android.graphics.Color.argb(options.color[0], options.color[1], options.color[2], options.color[3])), true, [ctx.getWindow().getDecorView(), android.view.Gravity.RIGHT | android.view.Gravity.TOP,dip2px(0),dip2px(0)]);
} catch(e) {
cm(e+", "+e.lineNumber);
}
}

function S_setNameDialog() {
ctx.runOnUiThread(new java.lang.Runnable({
run : function(){
try {
var dialog = new android.app.AlertDialog.Builder(ctx, 4);
...
var setN = new android.widget.EditText(ctx);
setN.setHint("서버 이름을 입력하세요..");
layout.addView(setN);

var scroll = android.widget.ScrollView(ctx);
scroll.addView(layout);
dialog.setView(scroll);
dialog.setTitle("서버 이름 설정");
dialog.setNegativeButton("취소", null);
dialog.setPositiveButton("확인", new android.content.DialogInterface.OnClickListener({
onClick : function(v){
R_Server.setName(setN.getText().toString());
toast("서버 이름을 "+setN.getText().toString()+"(으)로 바꾸었습니다.");
}
}));
dialog.show();
} catch(e) {
...
var setMP = makeEditText(android.text.InputType.TYPE_CLASS_NUMBER, " ", null, null);
setMP.setHint("서버 인원 제한수를 입력하세요..");
layout.addView(setMP);

var scroll = android.widget.ScrollView(ctx);
scroll.addView(layout);
dialog.setView(scroll);
dialog.setTitle("서버 인원 제한수 설절");
dialog.setNegativeButton("취소", null);
dialog.setPositiveButton("확인", new android.content.DialogInterface.OnClickListener({
onClick : function(v){
R_Server.setMaxPlayer(setMP.getText());
toast("서버 인원 제한수를 "+setMP.getText()+"(으)로 바꾸었습니다.");
}
}));
dialog.show();
} catch(e) {
...
function rethodFileDownload() {
ctx.runOnUiThread(new java.lang.Runnable({
run : function(){
try {
var dialog = new android.app.AlertDialog.Builder(ctx, 4);
dialog.setTitle("Rethod를 발견하지 못했습니다:(");
dialog.setMessage("Rethod 애드온이 없으면 팬동에디터를 이용하실 수 없습니다. 이동 버튼을 누르시면 MK 카페 Rethod 공유 페이지로 넘어갑니다.");
dialog.setNegativeButton("취소", null);
dialog.setPositiveButton("이동", new android.content.DialogInterface.OnClickListener({
onClick : function(v){
var intent = new android.content.Intent(android.content.Intent.ACTION_VIEW, android.net.Uri.parse("http://cafe.naver.com/minecraftpe/2608876"));
ctx.startActivity(intent);
}
}));
dialog.show();
...
var dexFile = new java.io.File(sdcard+"/games/com.mojang/RethodPEDex.dex");
if(!(appInstalled("cok.ljuwon.rethodpe")) && !(dexFile.exists())) rethodFileDownload();
}
